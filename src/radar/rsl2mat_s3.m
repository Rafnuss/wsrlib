...
cmd = sprintf('/usr/local/bin/aws s3 ls s3://noaa-nexrad-level2/%s/%s --no-sign-request', aws_path, name);
...
% Copy the scan
cmd = sprintf('/usr/local/bin/aws s3 cp s3://noaa-nexrad-level2/%s/%s %s --no-sign-request', aws_path, fullkey, local_file);
[status, result] = system(cmd);
if status
    error('command failed (exit status=%d)\n %s\n %s', status, cmd, result);
end
